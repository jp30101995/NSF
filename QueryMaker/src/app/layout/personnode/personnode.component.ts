...
@Component({
  ...
  templateUrl: './personnode.component.html',
  styleUrls: ['./personnode.component.scss']
})
export class PersonNodeComponent implements OnInit {
  ...
  deleteNode() {
    if (window.confirm('Are you sure you want to permanently delete this item?')) {
      this.treeService.deletePerson(this.person, this.person.parent);
      // .subscribe(
      //   res => {
  ...
  updatePerson(person_) {
    this.treeService.editPerson(this.person);
    // .subscribe(
    //   res => {
  ...
  private createPerson(person_: any) {
    this.treeService.addPerson(person_, this.person);
    // .subscribe(
    //   res => {
